...
export const AppWorkspace: React.FC<AppWorkspaceProps> = ({ onLogout }) => {
  ...
  const handleAddTransactionSubmit = (e: React.FormEvent) => {
    ...
    if (!txForm.amount || isNaN(parsedAmt) || parsedAmt <= 0) {
      alert('Please enter a valid positive transaction amount.');
      return;
    }
    const cents = Math.round(parsedAmt * 100);
    addTransaction(txForm.type, txForm.category, cents, txForm.description || 'Transaction Entry', txForm.date);
  ...
  const handleAddGoalSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const parsedTarget = parseFloat(goalForm.target);
    if (!goalForm.name || !goalForm.target || isNaN(parsedTarget) || parsedTarget <= 0) {
      alert('Please enter a valid goal name and positive target amount.');
      return;
    }
    const cents = Math.round(parsedTarget * 100);
